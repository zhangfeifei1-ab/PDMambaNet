...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.coarse_layers.append(VSSLayer_up(
            dim=int(dims[0] * 2),
            depth=2,
            d_state=math.ceil(dims[0] / 6) if d_state is None else d_state,
            drop=drop_rate, 
        ...
        self.coarse_output = nn.Conv2d(in_channels=int(dims[0]), out_channels=self.num_classes, kernel_size=1, bias=False)
        self.coarse_upsample = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False)
    ...
    def forward_coarse_decoder(self, x):
        for layer in self.coarse_layers:
            x = layer(x)
        x = x.permute(0, 3, 1, 2)  # B, C, H, W
        x = self.coarse_output(x)
        return x
    ...
    def forward(self, x):
        ...
        # Coarse decoder path
        coarse_output = self.forward_coarse_decoder(x_downsample[1])
        coarse_output = self.coarse_upsample(coarse_output)

        # Fine decoder path
        x = self.forward_up_features(x, x_downsample)
        fine_output = self.up_x4(x)
        
        out = fine_output + 0.9*coarse_output
